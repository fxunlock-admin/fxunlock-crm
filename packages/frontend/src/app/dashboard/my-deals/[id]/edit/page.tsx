  ...
  const loadDeal = async () => {
    try {
      ...
      setFormData({
        ...
        dealType: deal.dealType || 'REBATES',
        ftdsPerMonth: deal.ftdsPerMonth?.toString() || '',
        cpaTiers: deal.cpaTiers || [{ tierName: 'Tier 1', depositAmount: '', cpaAmount: '' }],
        expectedRoi: deal.expectedRoi?.toString() || '',
        netDepositsPerMonth: deal.netDepositsPerMonth?.toString() || '',
        expectedVolumeInLots: deal.expectedVolumeInLots?.toString() || '',
        rebatePerLot: deal.rebatePerLot?.toString() || '',
        pnlPercentage: deal.pnlPercentage?.toString() || '',
        region: deal.region || 'Global',
        instruments: deal.instruments || [],
        customInstrument: '',
        proofOfStats: null,
        additionalTerms: deal.additionalTerms || '',
        expiresInDays: Math.max(1, daysUntilExpiry).toString()
      })
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">Edit Deal Request</h1>
          <p className="text-gray-600 mt-2">Update your deal information</p>
        </div>
      </div>
        ...
        <div className="grid gap-6">
          ...
          <div className="flex gap-4">
            <Button type="submit" disabled={submitting} className="flex-1">
              {submitting ? 'Updating Deal...' : 'Update Deal'}
            </Button>
            <Link href="/dashboard/my-deals">
